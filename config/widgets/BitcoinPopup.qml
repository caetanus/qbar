...
Item {
    ...
    property bool showRsi: true          // Wilder RSI(14), overlaid on the volume pane
    ...
    readonly property int warmup: Math.max(bbPeriod - 1, rsiPeriod)
    ...
    function loadCandles() {
        ...
        var want = Math.min(1000, Math.max(120, root.barCount * 3 + root.warmup))
    ...
    function rebuildFromAll(jumpToLatest) {
        ...
        for (var i = 0; i < fcs.length; ++i)
            fcl.push(fcs[i].c)
        var bb = root.computeBollinger(fcl)
        var rsiValues = root.computeRsi(fcl)
    ...
    onDraftPointsChanged: drawingLayer.requestPaint()
    ...
    onShowBollingerChanged: { recomputeScale(); chart.requestPaint(); drawingLayer.requestPaint() }
    onChartTypeChanged: chart.requestPaint()
    onLogScaleChanged: { chart.requestPaint(); drawingLayer.requestPaint() }
    onShowVolumeChanged: { chart.requestPaint(); drawingLayer.requestPaint() }
    onShowRsiChanged: { chart.requestPaint(); drawingLayer.requestPaint() }
